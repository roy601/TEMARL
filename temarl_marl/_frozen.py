...
encoders and the GMU trunk from `thesis_system/temarl_v2/`. It does not copy
them: a copy could drift silently. Instead this module puts that directory on
...
def verify(verbose: bool = False) -> dict:
    ...
    fz = json.load(open(os.path.join(V2_DIR, "payoff_frozen.json"),
                        encoding="utf-8"))
